...
<head>
    <meta charset="UTF-8">
    <title>EcomPro — ${empty vente ? 'Nouvelle Vente' : 'Modifier Vente'}</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.2/css/bootstrap.min.css" rel="stylesheet"/>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" rel="stylesheet"/>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Space+Grotesk:wght@500;600;700&display=swap" rel="stylesheet"/>
    <style>
        :root { --primary:#6366f1; --success:#10b981; --danger:#ef4444; --dark:#0f172a; --text-muted:#94a3b8; --border:#e2e8f0; --bg:#f8fafc; }
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .form-control:focus, .form-select:focus { border-color:var(--success); box-shadow:0 0 0 3px rgba(16,185,129,0.1); }
        .form-actions { display:flex; gap:12px; margin-top:28px; padding-top:24px; border-top:1px solid var(--border); }
        .btn-save { background:var(--success); color:white; border:none; padding:10px 24px; border-radius:8px; font-size:0.875rem; font-weight:600; cursor:pointer; display:inline-flex; align-items:center; gap:8px; transition:all 0.2s; font-family:'Plus Jakarta Sans',sans-serif; }
        ...
        .breadcrumb-nav a { color:var(--success); text-decoration:none; }
        .stock-info { background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:12px 16px; margin-top:8px; font-size:0.82rem; color:#065f46; display:none; }
        .stock-warning { background:#fef3c7; border:1px solid #fde68a; border-radius:8px; padding:12px 16px; margin-top:8px; font-size:0.82rem; color:#92400e; display:none; }
    </style>
</head>
<body>
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="${pageContext.request.contextPath}/ventes" class="nav-link active"><i class="fa-solid fa-chart-line"></i> Ventes</a>
...
<div class="main-content">
    <div class="topbar">
        <div>
            <h2><i class="fa-solid fa-chart-line" style="color:#10b981;margin-right:8px;"></i>
                ${empty vente ? 'Nouvelle Vente' : 'Modifier Vente'}</h2>
            <p>${empty vente ? 'Enregistrer une nouvelle opération de vente' : 'Modifier l\'opération de vente'}</p>
        </div>
    </div>

    <div class="page-content">
        <div class="breadcrumb-nav">
            <a href="${pageContext.request.contextPath}/ventes"><i class="fa-solid fa-chart-line"></i> Ventes</a>
            <i class="fa-solid fa-chevron-right" style="font-size:0.7rem;"></i>
            <span>${empty vente ? 'Nouvelle' : 'Modifier'}</span>
        </div>

        <div class="form-card">
            <h3><i class="fa-solid fa-file-invoice-dollar" style="color:#10b981;margin-right:8px;"></i>
                Détails de la vente</h3>

            <form action="${pageContext.request.contextPath}/ventes" method="post">
                <c:if test="${not empty vente}">
                ...
                <div class="mb-3">
                    ...
                    <div class="stock-info" id="stockInfo">
                        ...
                        Stock disponible : <strong id="stockValue">0</strong> unités
                    </div>
                    <div class="stock-warning" id="stockWarning">
                        <i class="fa-solid fa-triangle-exclamation"></i>
                        Stock faible — vérifiez la quantité disponible
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="input-wrapper">
                        ...
                        <input type="number" name="quantity" class="form-control"
                               placeholder="ex: 10" value="${vente.quantity}" step="1" min="1" required/>
                    </div>
                </div>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn-save">
                        ...
                        ${empty vente ? 'Enregistrer la vente' : 'Modifier la vente'}
                    </button>
                    <a href="${pageContext.request.contextPath}/ventes" class="btn-cancel">
...
<script>
    function showStock(select) {
        ...
        if (!isNaN(stock) && select.value !== '') {
            val.textContent = stock;
            info.style.display = 'block';
            warning.style.display = stock <= 10 ? 'block' : 'none';
        } else {
