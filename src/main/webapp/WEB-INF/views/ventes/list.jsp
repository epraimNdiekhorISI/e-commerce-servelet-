...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-success-custom { background:var(--success); color:white; border:none; padding:9px 18px; border-radius:8px; font-size:0.85rem; font-weight:600; cursor:pointer; display:inline-flex; align-items:center; gap:7px; text-decoration:none; transition:all 0.2s; }
        .btn-success-custom:hover { background:#059669; color:white; box-shadow:0 4px 12px rgba(16,185,129,0.3); }
        ...
        .product-chip { display:inline-flex; align-items:center; gap:6px; background:#f0fdf4; color:#065f46; padding:4px 10px; border-radius:20px; font-size:0.78rem; font-weight:600; }
    </style>
</head>
<body>
<aside class="sidebar">
...
<div class="main-content">
    <div class="topbar">
        <div>
            <h2><i class="fa-solid fa-chart-line" style="color:#10b981;margin-right:8px;"></i>Ventes</h2>
            <p>Suivez toutes vos opérations de vente</p>
        </div>
        <a href="${pageContext.request.contextPath}/ventes?action=form" class="btn-success-custom">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <div class="value">${ventes.size()}</div>
                ...
                <div class="stat-card">
                    ...
                    <div class="value">
                        ...
                        <c:forEach items="${ventes}" var="v">
                            <c:set var="totalQte" value="${totalQte + v.quantity}"/>
                        </c:forEach>
                        ${totalQte}
                    </div>
                    <div class="label">Quantité totale vendue</div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stat-card">
        ...
        <div class="data-card">
            <div class="data-card-header">
                <div>
                    ...
                    <p>${ventes.size()} vente(s) enregistrée(s)</p>
                </div>
            </div>
            <table class="table">
                <thead>
                    <tr>
                        ...
                        <th>Quantité</th><th>Vendeur</th><th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:forEach items="${ventes}" var="v">
                                <tr>
                                    <td style="color:#94a3b8;font-size:0.8rem;">#${v.id}</td>
                                    <td>
                                        <span style="font-size:0.85rem;color:#475569;">
                                            <i class="fa-regular fa-calendar" style="margin-right:5px;color:#94a3b8;"></i>
                                            <fmt:formatDate value="${v.dateP}" pattern="dd/MM/yyyy"/>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="product-chip">
                                            <i class="fa-solid fa-box"></i> ${v.produitName}
                                        </span>
                                    </td>
                                    <td>
                                        <span style="font-weight:700;color:#065f46;">${v.quantity}</span>
                                    ...
                                    <td>
                                        <div style="display:flex;gap:6px;">
                                            <a href="${pageContext.request.contextPath}/ventes?action=form&id=${v.id}" class="btn-icon edit"><i class="fa-solid fa-pen"></i></a>
                                            <a href="${pageContext.request.contextPath}/ventes?action=delete&id=${v.id}" class="btn-icon delete" onclick="return confirm('Supprimer cette vente ?')"><i class="fa-solid fa-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            </c:forEach>
