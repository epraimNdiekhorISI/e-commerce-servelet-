...
<head>
    <meta charset="UTF-8">
    <title>EcomPro — Achats</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.2/css/bootstrap.min.css" rel="stylesheet"/>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" rel="stylesheet"/>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700&family=Space+Grotesk:wght@500;600;700&display=swap" rel="stylesheet"/>
    <style>
        :root { --primary:#6366f1; --primary-dark:#4f46e5; --info:#3b82f6;
                --danger:#ef4444; --dark:#0f172a; --text-muted:#94a3b8; --border:#e2e8f0; --bg:#f8fafc; }
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-primary-custom { background:var(--info); color:white; border:none; padding:9px 18px; border-radius:8px; font-size:0.85rem; font-weight:600; cursor:pointer; display:inline-flex; align-items:center; gap:7px; text-decoration:none; transition:all 0.2s; }
        .btn-primary-custom:hover { background:#2563eb; color:white; box-shadow:0 4px 12px rgba(59,130,246,0.3); }
        ...
        .product-chip { display:inline-flex; align-items:center; gap:6px; background:#eff6ff; color:#1d4ed8; padding:4px 10px; border-radius:20px; font-size:0.78rem; font-weight:600; }
    </style>
</head>
<body>
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="${pageContext.request.contextPath}/achats" class="nav-link active">
...
<div class="main-content">
    <div class="topbar">
        <div>
            <h2><i class="fa-solid fa-cart-shopping" style="color:#3b82f6;margin-right:8px;"></i>Achats</h2>
            <p>Suivez toutes vos opérations d'achat</p>
        </div>
        <a href="${pageContext.request.contextPath}/achats?action=form" class="btn-primary-custom">
        ...
        <div class="row g-3 mb-4">
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <div class="value">${achats.size()}</div>
            ...
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <div class="value">
                        ...
                        <c:forEach items="${achats}" var="a">
                            <c:set var="totalQte" value="${totalQte + a.quantity}"/>
                        </c:forEach>
                        ${totalQte}
                    </div>
                    <div class="label">Quantité totale achetée</div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stat-card">
                    ...
                    <div class="value">
                        <fmt:formatDate value="<%=new java.util.Date()%>" pattern="dd/MM"/>
                    </div>
        ...
        <div class="data-card">
            <div class="data-card-header">
                <div>
                    ...
                    <p>${achats.size()} achat(s) enregistré(s)</p>
                </div>
            </div>
            <table class="table">
                <thead>
                    <tr>
                        <th>#</th><th>Date</th><th>Produit</th>
                        <th>Quantité</th><th>Acheteur</th><th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:forEach items="${achats}" var="a">
                                <tr>
                                    <td style="color:#94a3b8;font-size:0.8rem;">#${a.id}</td>
                                    <td>
                                        <span style="font-size:0.85rem;color:#475569;">
                                            <i class="fa-regular fa-calendar" style="margin-right:5px;color:#94a3b8;"></i>
                                            <fmt:formatDate value="${a.dateP}" pattern="dd/MM/yyyy"/>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="product-chip">
                                            <i class="fa-solid fa-box"></i> ${a.produitName}
                                        </span>
                                    </td>
                                    <td>
                                        <span style="font-weight:700;color:#1d4ed8;">${a.quantity}</span>
                                        <span style="font-size:0.78rem;color:#94a3b8;"> unités</span>
                                    </td>
                                    <td style="color:#64748b;font-size:0.85rem;">
                                        <i class="fa-solid fa-user" style="margin-right:5px;color:#94a3b8;"></i>
                                        ${a.userEmail}
                                    </td>
                                    <td>
                                        <div style="display:flex;gap:6px;">
                                            <a href="${pageContext.request.contextPath}/achats?action=form&id=${a.id}" class="btn-icon edit"><i class="fa-solid fa-pen"></i></a>
                                            <a href="${pageContext.request.contextPath}/achats?action=delete&id=${a.id}" class="btn-icon delete" onclick="return confirm('Supprimer cet achat ?')"><i class="fa-solid fa-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            </c:forEach>
